...
ai-automation service-domain MCP for the Orryx Group. Built on `@orryx/core/domain-mcp-base`.
...
- ``/compute-cost-estimate`` (compute)
- ``/sync-workflow-status`` (sync)

## Domain → port

`ai-automation` = `9009` (per the canonical port map in `@orryx/core/domain-mcp-base/README.md`).
